...
- [ ] **Hosting**: Render **web service on the FREE tier** (same as the other 7 Render services). No persistent disk, no paid plan. Deploy is **dashboard-driven** (confirmed) → the M3 cron decommission is a manual dashboard delete of the `Sync-Scansia` cron. Deploy config: `buildCommand: pip install -r requirements.txt` · `startCommand: uvicorn backend.app:app --workers 1 --host 0.0.0.0 --port $PORT` · `healthCheckPath: /health` (ungated) · Python pinned via **`.python-version` = 3.12.4** (Render does NOT honor `runtime.txt` — added 2026-07-07 so future deploys don't depend on the manually-set `PYTHON_VERSION` env var; `runtime.txt` kept for local pyenv).
